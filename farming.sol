...
// Inheritance
contract Farming is Owned, Pausable {
    using SafeMath for uint256;
    ...
    uint256 public totalRewardsPaidout = 0;
    uint256 public stakesDone = 0;
    
    uint256 public minimunStake1 = 1 *1e18;
    uint256 public divider = 90; // 400% yearly
    
    function updateMinimumStakeAmount(uint256 amount) public onlyOwner{
        minimunStake1 = amount;
    }
    
    function updateDivider(uint256 num) public onlyOwner{
        divider = num;
    }
    ...
    IERC20 public stakingToken = IERC20(0x3DA3f053181B0D9871451634E512C315443ab622);
    IERC20 public rewardToken = IERC20(0xfCe3b7abBED6e7e5B1d3e85d4a369D7658f0190E);
    ...
    function stake(uint256 amount) external {
        ...
        user[msg.sender].lockTime1 = getTime();
        
        totalStaked1 = totalStaked1 + amount;
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        emit Staked(msg.sender, amount);      
        stakesDone = stakesDone + 1;
    }

    function updateReward() internal {
        user[msg.sender].rewardBal = user[msg.sender].rewardBal + IntervalRewardsOf1(msg.sender);
    }
     ...
     function IntervalRewardsOf1(address account) public view returns (uint256) {
         ...
         uint256 perIntervalReward = amount.div(divider); 
    ...
    function withdrawReward1() public{
        ...
        rewards = rewards + user[msg.sender].rewardBal;
        user[msg.sender].rewardBal = 0;
        //require(rewards > 0, "Account does not have reward balance staked");
        if(rewards > 0){
            ////////////////////////////////////////////
            uint256 timeDiff = getTime().sub(user[msg.sender].stakedTime1);
            uint256 intervals = timeDiff.div(rewardInterval);
            user[msg.sender].stakedTime1 = user[msg.sender].stakedTime1 + (intervals.mul(86400));
        ///////////////////////////////////////////        
        totalRewardsPaidout = totalRewardsPaidout + rewards;
        ///////////////////////////////////////////////
